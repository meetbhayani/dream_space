...
const consultationSchema = z.object({
  ...
  consultationType: z.enum(['virtual', 'in-person']),
  projectType: z.enum(['villa', 'apartment', 'commercial', 'renovation', 'other']),
  ...
  if (status === 'success') {
    return (
      <div className="flex flex-col items-center justify-center rounded-[24px] border border-white/10 bg-white/5 p-10 text-center">
        ...
        <h3 className="text-xl text-white">Consultation booked</h3>
        <p className="mt-2 text-sm text-muted">We&apos;ll confirm your slot within 24 hours.</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className={`space-y-5 ${className}`}>
      <div className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className="mb-2 block text-xs uppercase tracking-[0.2em] text-muted">Preferred date</label>
            <input
          ...
          <div>
            <label className="mb-2 block text-xs uppercase tracking-[0.2em] text-muted">Time slot</label>
            <select
            ...
            {errors.preferredTimeSlot && <p className="mt-1 text-xs text-red-400">{errors.preferredTimeSlot.message}</p>}
          </div>
        </div>

        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className="mb-2 block text-xs uppercase tracking-[0.2em] text-muted">Consultation type</label>
            <select
            ...
            {errors.consultationType && <p className="mt-1 text-xs text-red-400">{errors.consultationType.message}</p>}
          </div>
          <div>
            <label className="mb-2 block text-xs uppercase tracking-[0.2em] text-muted">Project type</label>
            <select
